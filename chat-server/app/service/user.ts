...
export default class User extends Service {
  // 查询用户
  public async retrieve(user) {
    const { ctx } = this;
    const { username, email = '', phone = '' } = user;
    const dbUser = await ctx.model.User.findOne({
      // or 查询
  ...
  public async searchQuery(userInfo: any) {
     ...
     */
    const conditionQuery = await ctx.model.User.find({ userInfo });
  ...
  public async create(user) {
    ...
    // 密码加密
    user.password = await ctx.helper.md5(user.password);
    // 保存数据
  ...
  // 更新用户
  public async update(id: number, userInfo: any) {
    const { ctx } = this;
